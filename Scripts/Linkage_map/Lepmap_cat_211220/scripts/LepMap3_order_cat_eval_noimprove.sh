...
OUTPUT_PATH=$INPUT_PATH/OrderMarker_int_no_improve
...
ORDER_FILE=$(awk -v i=$lg 'NR==i' reordermarkers/best.likelihoods)

zcat $INPUT_DATA |java -Xmx2048m -cp LM3 OrderMarkers2\
...
awk -v lg=$lg '{print $4, $5, $6=lg, $1, $2, $3}' $OUTPUT_PATH/order_${lg}_intervals.mapped.txt | sed 's/HiC_scaffold_//g' |sed 's/\*//g' |uniq >> $OUTPUT_PATH/order_all_int.txt
done
