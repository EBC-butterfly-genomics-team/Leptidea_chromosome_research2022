...
# script by Tom Poorten, dotplotly https://github.com/tpoorten/dotPlotly
# (adapted by L. Höök)
...
option_list <- list(
  make_option(c("-f", "--ref"), type="character",
	      help="set reference label"),
  make_option(c("-e", "--que"), type="character",
	      help="set query label"),
  make_option(c("-a", "--annotation"), type="numeric",
	      help="add annotation",
...
# set annotations
#if (opt$annotation) {
#	annotate = read.table(opt$annotation_file, stringsAsFactors = F, header = T)
#}

#annotate = read.table(opt$annotation_file, stringsAsFactor = F, header = T)
#annotate <- annotate[which(annotate$scaffold==opt$refIDs),]
...
if (opt$similarity) {
  gp = ggplot(alignments) +
    ...
    scale_x_continuous(breaks = cumsum(as.numeric(chromMax)), expand = c(0,0),
                       labels = levels(alignments$refID), guide = guide_axis(n.dodge = 2)) +
    theme_bw() +
    ...
    scale_y_continuous(breaks = yTickMarks, expand = c(0,0), labels = substr(levels(alignments$queryID), start = 1, stop = 20),
			guide = guide_axis(n.dodge = 2)) +
    { if(opt$h_lines){ geom_hline(yintercept = yTickMarks,
                                  color = "grey60",
                                  size = .1) }} +
    scale_color_distiller(palette = "Reds", limits = c(90, 100), direction = 1) +
    labs(color = "Mean Query\n% Identity") +
    xlab(opt$ref) +
    ylab(opt$que) +

#chr 18: -1913933
    
#    geom_point(data = annotate, x = annotate$position, y = annotate$position, size = 2.5, color = "darkgreen") +
#    geom_segment(data = annotate, x = annotate$position-100000, xend = annotate$position+100000, y = annotate$position, yend = annotate$position, size = 1.5, color = "darkgreen") +
